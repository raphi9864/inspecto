    ...
    <div ref={containerRef}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn-outline" title="Export PNG" onClick={() => { exportPNG(containerRef, `inspecto-gantt-${new Date().toISOString().slice(0, 10)}.png`); showToast(t('toasts.pngExported'), 'success') }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
            {t('gantt.png')}
          </button>
          <button className="btn-outline" title="Export Excel" onClick={() => { const exportData = rows.map(r => ({ label: r.label, start: r.start, duration: r.duration, progress: r.progress != null ? r.progress : '' })); exportXLSX(exportData, `inspecto-gantt-${new Date().toISOString().slice(0, 10)}.xlsx`); showToast(t('toasts.excelExported'), 'success') }}>
        ...
        <div className="modal-overlay" onClick={() => setSelectedTask(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="btn-primary" onClick={() => { showToast(t('toasts.progressUpdated')); setSelectedTask(null) }}>{t('common.save')}</button>
            </div>
          </div>
        </div>
      )}
